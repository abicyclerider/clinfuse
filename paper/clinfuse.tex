...
Patients with duplicate medical records are five times more likely to die during hospitalization than those with a single, unified chart~\cite{western2026}. In one case, a care team followed a do-not-resuscitate order from the wrong patient's record during a cardiac arrest---withholding life-saving treatment~\cite{ecri2016}. Across 7{,}613 wrong-patient events at 181 healthcare organizations, 9\% resulted in patient harm~\cite{ecri2016}.

An estimated 8--12\% of hospital records are duplicates~\cite{morris2016}, costing the U.S.\ healthcare system \$6.7 billion annually~\cite{blackbook2018}. Match accuracy averages 80--90\% within institutions but falls to 50\% in cross-organizational exchange~\cite{onc2014}. The MATCH~IT Act (H.R.~2002, 2025) sets a 99.9\% matching target~\cite{matchit2025}, while TEFCA v2.1 makes cross-organizational matching critical national infrastructure~\cite{tefca2024}. Fragmented records also bias health research through linkage-error-driven misclassification~\cite{harron2019}.
...
Prior work has used diagnosis codes for de-identified linkage~\cite{medlink2023} and LLMs for clinical trial matching~\cite{prism2024}, but no published system combines LLM-based clinical interpretation with probabilistic linkage for patient entity resolution.

MedGemma~4B~\cite{medgemma}---a medical foundation model pretrained on clinical text---provides exactly this missing clinical reasoning in an edge-deployable package. ClinFuse demonstrates this capability by fusing Splink probabilistic matching with a fine-tuned MedGemma classifier, achieving 99.7\% F1 with perfect recall on synthetic evaluation data (Section~\ref{sec:results})---demonstrating that MedGemma's clinical understanding can close the gap between current cross-organizational match rates and the 99.9\% policy target. With the duplicate problem costing \$6.7B annually, even incremental matching improvements translate to significant cost savings and prevented patient harm.
